...
export class FacultadComponent {
  ...
  columns: Column[] = [
    { columnDef: 'id', header: 'No.', cell: (element: Facultad) => `${element.id}` },
    { columnDef: 'nombre', header: 'Unidad', cell: (element: Facultad) => `${element.nombre}` },
  ...
  async cargartabla(){
    try {
      const source$ = this.facultadApi.listar();
  ...
  private mostrarErrorAlCargar(error: unknown): void {
    ...
    Swal.fire({
      ...
      position: 'top-end',
      icon: 'error',
      title: 'No se pudo actualizar la lista de unidades',
  ...
  eliminar(element: any){
    this.facultadApi.eliminar(element.id).subscribe(data=>{
      Swal.fire({
        title: "Eliminado",
        text: "Continuar",
        icon: "info"
